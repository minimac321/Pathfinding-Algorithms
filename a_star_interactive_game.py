# Adapted from Tutorial: https://www.youtube.com/watch?v=bZkzH5x0SKU
# Credits too https://github.com/StanislavPetrovV/Python-Dijkstra-BFS-A-star
...
from ui_utils import update_screen_execution_time, update_screen_path_cost, update_screen_nodes_searched, \
    set_screen_with_image, get_click_mouse_pos, update_screen_with_all_node, draw_circle_nodes


def get_shortest_a_star_path(graph: dict, start_node: tuple[int, int], goal_node: tuple[int, int]) -> Optional[dict]:
    frontier = []
    ...
    while frontier:
        ...
        for neigh_node_cost, neigh_node in adjacent_nodes:
            new_node_cost = neigh_node_cost + min_cost_to_node_so_far[curr_node]
            ...
            if neigh_node not in min_cost_to_node_so_far or new_node_cost < min_cost_to_node_so_far[neigh_node]:
                ...
                heapq.heappush(frontier, (priority, neigh_node))
    ...
    if current_node is not None:
        shortest_path = get_shortest_path_from_visited_graph(node_came_from_node,
...
def driver(show_grid=False):
    ...
    # Set UI
    set_screen_with_image(screen, show_grid)
    ...
    while True:
        ...
        if mouse_pos:
            ...
            # Get A-star path
            path_dict = get_shortest_a_star_path(
